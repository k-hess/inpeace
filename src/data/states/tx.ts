...
const smallEstateAffidavit: Rule = {
  ...
  copy: (ctx: RuleContext) => {
    ...
    return {
      ...
      body: notSure
        ? `If it turns out there wasn't a will, and what's left (not counting the house) adds up to $75,000 or less, Texas has a simpler option called a small estate affidavit. A judge still has to approve it, and it can't be filed until ${availableDate}, thirty days out. Nothing to act on yet, good to know it exists.`
...
const creditorWindow: Rule = {
  ...
  copy: (ctx: RuleContext) => {
    ...
    return {
      ...
      body: `Texas gives creditors a general period to make claims against the estate, typically 4 to 6 months out, so roughly ${start} through ${end}. This is part of why it's worth waiting before distributing anything, not something you need to track day by day.`,
    }
  },
}
